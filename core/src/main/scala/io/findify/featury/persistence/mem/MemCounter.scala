package io.findify.featury.persistence.mem

import com.github.blemale.scaffeine.Cache
import io.findify.featury.model.Feature.Counter
import io.findify.featury.model.FeatureConfig.CounterConfig
import io.findify.featury.model.FeatureValue.{Num, ScalarValue}
import io.findify.featury.model.{FeatureValue, Key, WriteRequest}

case class MemCounter(config: CounterConfig, cache: Cache[Key, Long]) extends Counter {
  override def put(action: WriteRequest.Increment): Unit = {
    cache.getIfPresent(action.key) match {
      case Some(counter) => cache.put(action.key, counter + action.inc)
      case None          => cache.put(action.key, action.inc)
    }

  }
  override def computeValue(key: Key): Option[FeatureValue.ScalarValue[FeatureValue.Num]] = {
    cache.getIfPresent(key).map(c => ScalarValue(Num(c)))
  }
}
