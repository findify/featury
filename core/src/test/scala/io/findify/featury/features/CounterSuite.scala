...
import io.findify.featury.model.WriteRequest.Increment
...
trait CounterSuite extends FeatureSuite[CounterConfig, Counter] {
  ...
  it should "increment once" in withFeature { counter =>
    ...
    counter.put(Increment(key, 1))
    counter.computeValue(key) shouldBe Some(ScalarValue(1.0))
  }
  ...
  it should "inc-dec multiple times" in withFeature { counter =>
    ...
    increments.foreach(inc => counter.put(Increment(key, inc)))
