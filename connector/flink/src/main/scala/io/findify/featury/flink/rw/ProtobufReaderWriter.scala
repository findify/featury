...
trait ProtobufReaderWriter[T <: GeneratedSealedOneof] {
  def writeFile(
      path: Path,
...
object ProtobufReaderWriter {
  trait ProtobufStreamFormat[T <: GeneratedSealedOneof] extends StreamFormat[T] {
  ...
  case class ProtobufStreamFormatReader[T <: GeneratedSealedOneof](
      stream: InputStream,
      make: InputStream => T
  ...
  trait ProtobufBulkFormat[T <: GeneratedSealedOneof] extends BulkFormat[T, FileSourceSplit] {}

  case class ProtobufBulkWriterFactory[T <: GeneratedSealedOneof](compress: Compress) extends BulkWriter.Factory[T] {
    class ProtobufBulkWriter(out: BufferedOutputStream) extends BulkWriter[T] {
      override def addElement(element: T): Unit = element.asMessage.writeDelimitedTo(out)
  ...
  trait ProtobufBucketAssigner[T <: GeneratedSealedOneof] extends BucketAssigner[T, String] {
